...
import com.easytestit.generatexml.parser.internal.JsonReaderInternalAccess;
...
public final class MapTypeAdapterFactory implements TypeAdapterFactory {
    ...
    @Override public <T> TypeAdapter<T> create(JsonParser jsonParser, TypeToken<T> typeToken) {
        ...
        TypeAdapter<?> keyAdapter = getKeyAdapter(jsonParser, keyAndValueTypes[0]);
        TypeAdapter<?> valueAdapter = jsonParser.getAdapter(TypeToken.get(keyAndValueTypes[1]));
        ObjectConstructor<T> constructor = constructorConstructor.get(typeToken);

        @SuppressWarnings({"unchecked", "rawtypes"})
        // we don't define a type parameter for the key or value types
                TypeAdapter<T> result = new Adapter(jsonParser, keyAndValueTypes[0], keyAdapter,
     ...
     */
    private TypeAdapter<?> getKeyAdapter(JsonParser context, Type keyType) {
    ...
    private final class Adapter<K, V> extends TypeAdapter<Map<K, V>> {
        ...
        public Adapter(JsonParser context, Type keyType, TypeAdapter<K> keyTypeAdapter,
