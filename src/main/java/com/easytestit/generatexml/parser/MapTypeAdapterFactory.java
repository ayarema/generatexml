...
public final class MapTypeAdapterFactory implements TypeAdapterFactory {
    ...
    @Override public <T> TypeAdapter<T> create(Json json, TypeToken<T> typeToken) {
        ...
        TypeAdapter<?> keyAdapter = getKeyAdapter(json, keyAndValueTypes[0]);
        TypeAdapter<?> valueAdapter = json.getAdapter(TypeToken.get(keyAndValueTypes[1]));
        ObjectConstructor<T> constructor = constructorConstructor.get(typeToken);

        @SuppressWarnings({"unchecked", "rawtypes"})
        // we don't define a type parameter for the key or value types
                TypeAdapter<T> result = new Adapter(json, keyAndValueTypes[0], keyAdapter,
     ...
     */
    private TypeAdapter<?> getKeyAdapter(Json context, Type keyType) {
    ...
    private final class Adapter<K, V> extends TypeAdapter<Map<K, V>> {
        ...
        public Adapter(Json context, Type keyType, TypeAdapter<K> keyTypeAdapter,
